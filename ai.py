...
class DQN(object):  # 强化神经网络
    def __init__(self, id):
        ...
        self.epsilon = 1.0
        self.memory = np.zeros((MEMORY_CAPACITY, N_STATES * 2 + 1 + N_ACTIONS))  # 初始化记忆全为 0
        ...
        # 读取记忆
        try:
            self.memory = np.loadtxt("memory" + id + ".txt", delimiter=',')
            valid = len(self.memory[np.any(self.memory != 0, axis=1)])
            print("[有效记忆]", valid)
            self.memory_counter = valid
            print("加载记忆成功")
    ...
    def choose_action(self, x):  # 传入当前的 State，计算行为
        ...
        # 只输入一个样本
        if self.memory_counter >= MEMORY_CAPACITY and np.random.uniform() < self.epsilon:  # 90% 概率使用评估网络的结果
    ...
    def store_transition(self, s, a, r, s_):
        ...
        if s is None or a is None or s_ is None:
            return
        v = (*[i.item() for i in s], *list(a), r, s_)
        # print("\n", "Store: action:", len(a))
        transition = np.hstack(v)
        # 用新内存替换旧内存
    ...
    def learn(self):
        ...
        # q_eval w.r.t体验中的行动
        q_eval_temp = self.eval_net(b_s)
        # print("[Q_Eval]:", q_eval_temp)
        # print("[Batcg_action]:", b_a.shape)
        # torch.set_printoptions(profile="full")
        # print("[Batcg_action]:", b_a)
        q_eval = q_eval_temp.gather(1, b_a)  # shape (batch, 1)
    ...
    def save(self, id):
        # print("\n\n[Memory]\n", self.memory)
        np.savetxt('memory' + id + '.txt', self.memory, fmt="%d", delimiter=',')
...
class AI(object):
    def __init__(self, id):
        ...
        try:
            print("读取模型中...")
            self.dqn.target_net = torch.load("target-" + id + ".pth")
    ...
    def resp(self, resp_):  # 存储响应
        ...
        self.times += 1
        self.ep_r += reward
        print("[Memory]", self.dqn.memory_counter, "/", MEMORY_CAPACITY)
    ...
    def save(self, id):
        print("保存模型中...")
        torch.save(self.dqn.target_net, "target-" + id + ".pth")
        torch.save(self.dqn.eval_net, "eval-" + id + ".pth")
        print("保存记忆中")
        self.dqn.save(id)
        print("保存成功，程序已退出")
